...
def bench_cpu_gemm(sizes):
    ...
    for n in sizes:
        ...
        print(f"    {n:>5}x{n:<5}  {t:8.4f}s  {gf:8.2f} GFLOPS")
    return results
...
def bench_cpu_batch_matmul(batch=64, seq=512, dim=64):
    ...
    print(f"    Batch MM (B={batch} S={seq} D={dim})  {t:.4f}s  {gf:.2f} GFLOPS")
...
def bench_cpu_membw(size_mb=256):
    ...
    bw = size_mb / t / 1024
    print(f"    Mem BW ({size_mb} MB copy)  {t:.4f}s  {bw:.2f} GB/s")
...
def bench_gpu_gemm(sizes):
    ...
    for n in sizes:
        ...
        print(f"    {n:>5}x{n:<5}  {t:8.4f}s  {gf:8.2f} GFLOPS  mem:{mem}MB")
        del a, b
    return results
...
def bench_gpu_batch_matmul(batch=64, seq=512, dim=64):
    ...
    gf = 2 * batch * seq * dim * seq / t / 1e9
    mem = _mps_mem_mb()
    print(f"    Batch MM (B={batch} S={seq} D={dim})  {t:.4f}s  {gf:.2f} GFLOPS  mem:{mem}MB")
    del a, b
...
def bench_gpu_conv2d():
    ...
    mem = _mps_mem_mb()
    print(f"    Conv2D (32x64x56x56 -> 128ch k=3)  {t:.4f}s  mem:{mem}MB")
    del conv, x
...
def bench_gpu_transfer():
    ...
    print(f"    CPU->GPU ({size_mb}MB)  {t_h2d:.4f}s  {bw_h2d:.2f} GB/s")

    gpu_t = cpu_t.to(dev)
    _sync()
    t_d2h = bench(lambda: gpu_t.cpu(), device="mps")
    bw_d2h = size_mb / t_d2h / 1024
    print(f"    GPU->CPU ({size_mb}MB)  {t_d2h:.4f}s  {bw_d2h:.2f} GB/s")
...
def bench_gpu_fp16_fp32():
    ...
    print(f"    FP32  {n}x{n}  {t32:.4f}s  {gf32:.2f} GFLOPS")
    print(f"    FP16  {n}x{n}  {t16:.4f}s  {gf16:.2f} GFLOPS")
...
def bench_seq_scaling(d_model=1024, nhead=8, batch=8, device_name="mps"):
    ...
    for seq in [128, 256, 512, 1024, 2048]:
        ...
        try:
            ...
            print(f"    seq={seq:>5}  {t:.4f}s  {tok:>8.0f} tok/s  mem:{mem}MB")
            del block, x
            _mps_reset()
        except RuntimeError as e:
            if "out of memory" in str(e).lower() or "MPS" in str(e):
                print(f"    seq={seq:>5}  SKIPPED (OOM)")
...
def bench_lora(d_model=1024, nhead=8, batch=8, seq=512, rank=16, device_name="mps"):
    ...
    try:
        ...
        # Low-rank adapters for Q, K, V, and output projections
        ...
        def lora_step():
            ...
            for p in lora_b.values():
                if p.grad is not None:
                    p.grad.zero_()
            out = block(x)
            # LoRA residual (simplified: apply to block output)
        ...
        print(f"    Full FT:       {tok_full:>8.0f} tok/s  mem:{mem_full}MB  params:{full_params:,}")
        print(f"    LoRA (r={rank}):   {tok_lora:>8.0f} tok/s  mem:{mem_lora}MB  params:{lora_params:,}")
        print(f"    LoRA speedup: {speedup:.2f}x   memory saving: {mem_save:.0f}%")
...
# (name, params_B, layers, d_model, nhead)
MODEL_CONFIGS = [
...
def model_feasibility(ram_gb):
    ...
    avail = ram_gb * 0.75  # conservative usable fraction
    results = []

    print(f"\n    Available memory (est.): {avail:.1f} GB of {ram_gb:.1f} GB total\n")
    ...
    for name, params_b, layers, d, nh in MODEL_CONFIGS:
        params = params_b * 1e9
        fp16_gb = params * 2 / 1e9                      # weights only (FP16)
        train_gb = params * 20 / 1e9                     # weights + grads + Adam
        lora_gb = (params * 2 + params * 0.01 * 20) / 1e9  # frozen FP16 + LoRA optimizer

        def _tag(gb):
            mark = "\u2713" if gb < avail else "\u2717"
            return f"{mark} {gb:>5.1f}GB"
...
def find_max_batch(d_model=1024, nhead=8, seq=512, device_name="mps"):
    """Binary-search for the largest training batch that fits in memory."""
    dev = torch.device(device_name)
    max_b = 0
    for b in [1, 2, 4, 8, 16, 32, 64, 128, 256]:
        ...
        try:
            ...
            _mps_reset()
            break
    print(f"    Max batch (d={d_model}, seq={seq}): {max_b}")
    return {"d_model": d_model, "seq": seq, "max_batch": max_b}


def estimate_training_times(transformer_results, device_name):
    """Project per-step training time for popular models using measured block throughput."""
    ...
    results = []
    steps = 1000
    print(f"\n    Projected for {steps} training steps (single block scaled by layers):\n")
    print(f"    {'Model':<15} {'Step':>8}  {'Steps/hr':>9}  {'1K steps':>10}")
    print(f"    {'':->15} {'':->8}  {'':->9}  {'':->10}")

    for name, params_b, layers, d, nh in MODEL_CONFIGS:
        closest = min(block_times, key=lambda k: abs(k - d))
        # Scale: layers * (d/closest)^2  (rough FLOP scaling)
        ...
        print(f"    {name:<15} {step_t:>6.3f}s  {sph:>9.0f}  {fmt:>10}")
        results.append({
...
def bench_inference_tok(configs, device_name):
    ...
    for d_model, nhead, batch, seq in configs:
        ...
        try:
            ...
            # Decode: single-token forward (simulates autoregressive step)
...
def bench_disk_io(size_mb=512):
    ...
    try:
        ...
        print(f"    Seq Write ({size_mb}MB)  {t_w:.3f}s  {bw_w:.2f} GB/s")
        ...
        print(f"    Seq Read  ({size_mb}MB)  {t_r:.3f}s  {bw_r:.2f} GB/s")
...
# Approximate ML performance multipliers relative to M1 (7-8 GPU core base).
# Based on GPU core counts, arch improvements, and community benchmarks.
# Run the benchmark on each machine for real numbers.
REFERENCE_CHIPS = [
    ("M1",        1.0,  "7-8",   2020),
...
def compute_score(transformer_results):
    """Primary score = transformer training tok/s at d=1024 (or nearest)."""
    for r in transformer_results:
...
def print_comparison(score, chip_name):
    ...
    # Detect which chip this is
    detected = None
    chip_lower = chip_name.lower().replace(" ", "")
    # Sort references longest-name-first so "M1 Pro" matches before "M1"
    ...
    if detected:
        base_score = score / detected[1]  # M1-equivalent
    else:
        base_score = score  # assume M1-class

    max_mult = max(r[1] for r in REFERENCE_CHIPS)
    bar_w = 35

    print(f"\n    Your score: {score:,.0f} tok/s  (transformer training, d=1024)\n")
    ...
    for ref_name, mult, cores, year in REFERENCE_CHIPS:
        ...
        bar = "\u2588" * bar_len
        tag = " <- you" if detected and ref_name == detected[0] else ""
        print(f"    {ref_name:<12} {cores:>10} {est:>11,}  {rel:>5.1f}x  {bar}{tag}")

    print(f"\n    Cloud GPU Equivalence (approximate):")
    for name, mult, mem in CLOUD_GPUS:
        if detected:
            pct = detected[1] / mult * 100
        else:
            pct = 1.0 / mult * 100
        print(f"      ~{pct:.1f}% of {name}")
...
def main():
    ...
    W = 65
    print("=" * W)
    print("  Apple Silicon ML Benchmark")
    print("=" * W)
    ...
    if has_mps:
        ...
        print("\n[GPU SKIPPED - MPS not available]")

    # ---- transformer ----
    print(f"\n[Transformer Benchmarks ({device.upper()})]")

    print("  Training + inference throughput")
    res["transformer"]["throughput"] = bench_transformer_throughput(transformer_cfgs, device)

    if not args.quick:
        print("\n  Sequence length scaling (d=1024, batch=8)")
        res["transformer"]["seq_scaling"] = bench_seq_scaling(device_name=device)

        print("\n  LoRA vs full fine-tuning (d=1024)")
